...
from app.agents.synthesizer import _append_evidence_appendix, _render_context_block
...
def test_appendix_includes_grade_summary_when_nonzero():
    out = _append_evidence_appendix("Body.", {"evidence_distribution": DIST}, [], [])
...
def test_appendix_omits_grade_summary_when_absent():
    out = _append_evidence_appendix("Body.", {}, [], [])
    assert "Evidence grades:" not in out


def test_appendix_omits_grade_summary_when_all_zero():
    out = _append_evidence_appendix(
        "Body.", {"evidence_distribution": {"A": 0, "B": 0, "C": 0, "D": 0}}, [], []
    )
    assert "Evidence grades:" not in out
