...
| LLM response cache | 66% hit ratio on repeat-heavy workload, 6.26x repeat-pass speedup |
| End-to-end pipeline (mocked LLM) | 2 iterations (intelligent stop), 2 dependency waves, support rate 1.0000 |
...
Per-claim latency 152.9 µs. Misses: 0.
...
4.0200 ms for 12 labeled pairs.

### Confidence engine v2

Scores: {'strong': 0.823, 'moderate': 0.714, 'weak': 0.283, 'conflicted': 0.75}. Contradictions apply a pool-size-scaled penalty;
...
| Operation | Latency |
|---|---|
| Similarity matrix, 60 facts | 365.7 ms |
| Similarity matrix, 200 facts | 924.8 ms |
| Cross-similarity, 40 sentences x 200 claims | 16.0 ms |
| Contradiction scan, 60 facts | 406.5 ms |
| Dedup pass, 60 facts | 396.0 ms |
| Answer-support check, 40 sentences | 0.4000 ms |
| Pair similarity (single) | 140.7 µs |

Peak-RSS delta across a 300-fact matrix workload: 21804 KiB —
